...
for i in range(0, len(DLM)):
	for j in range(0, len(M1)):
		...
		Lh= 25.0
		Lmid=(Ll+Lh)/2
		FILENAME= "../../HOD_new/output500/galaxies_V1.0_M1_%.2f_DLM%.2f_Lth%.2f.dat" % (M1[j], DLM[i], Ll)
		...
		if ( (N<= Ngal- tol) ):
			...
			np.savetxt("New_HODdata_500/1e5galaxies_V1.0_M1_%.2f_DLM%.2f.dat"%(M1[j], DLM[i]),out)
